    ...
    if (postbackPayload || text) {
      if (['做功德', '上班'].indexOf(text) >= 0 || postbackPayload === P.CHECK_IN) {
        if (!context.state.isWorking) {
          ...
          await context.sendText('不如再按下 “Send Location” 傳送你的位置吧，我們會幫你記錄下你工作的位置！', genQuickReply([P.SEND_LOCATION, P.CHECK_OUT]));
        } else {
          // state: is working
          if (!context.state.location) {
            await context.sendText('哎呀！ 你居然不知道你已經在做功德了！？ 不如這樣，順便按下 “Send Location” 傳送你的位置吧，我們會幫你記錄下你工作的位置！', genQuickReply([P.SEND_LOCATION, P.CHECK_OUT]));
          } else if (context.state.imgUrls.length === 0) {
            await context.sendText('唔... 你其實已經在做功德了！ 不如這樣，上傳幾張可以證明你在工作的照片，我們會幫你存起來！', genQuickReply([P.CHECK_OUT]));
          }
        }
        ...
        if (context.state.isWorking) {
          ...
          await context.sendText(`你今天總共生產了 ${formatTime(calcTime(time))} 的功德，已經存到台灣功德資料庫內。\n\n台灣因為有你的功德，才能有今日亮眼的經濟成績。\n善哉善哉，讚嘆、感恩施主。`, genQuickReply([P.CHECK_IN]));
      ...
      if (context.state.isWorking) {
        ...
        console.log('after setting state:', context.state);
        await context.sendText('唔... 真是神秘的功德產生地。\n不過地點已經成功儲存了，現在不如傳幾張可以證明你在工作的照片吧！', genQuickReply([P.CHECK_OUT]));
      } else {
        context.setState({
          isWorking: true,
          startTime: new Date(),
        ...
        await context.sendText('工作位置已記錄，開啟做功德模式！ 現在不如傳幾張可以證明你在工作的照片吧！', genQuickReply([P.CHECK_OUT]));
      }
    } else if (imgUrl) {
      ...
      if (context.state.isWorking) {
        ...
        console.log('after setting state:', context.state);
        if (!context.state.location) {
          await context.sendText('感謝大大無私奉獻，需要上傳更多張也沒問題！\n然後，不如再按下 “Send Location” 傳送你的位置吧！', genQuickReply([P.SEND_LOCATION,P.CHECK_OUT]));
        } else {
          await context.sendText('感謝大大無私奉獻！ 能證明你在工作的照片已儲存，需要上傳更多張也沒問題！', genQuickReply([P.CHECK_OUT]));
        }
      }
    }
  });
